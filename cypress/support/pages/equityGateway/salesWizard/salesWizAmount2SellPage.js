...
class salesWizAmount2SellPage extends BasePage {
  ...
  certificatesDisable(certificateName, certificatePosition) {
    ...
    cy.contains(certificateName)
      .get(selectors.certificateName + ' > div > div > div > div')
      .should('have.css', 'background-color', 'rgb(255, 202, 95)')
      .click()
  ...
  certificatesModalValidation(certificateName, availability, certificatePosition) {
    const elements = ['Edit shares selection', 'Available', 'Estimated Proceeds', 'Estimated Gain/Loss', 'Dismiss', 'Save']
    ...
    if (certificatePosition !== undefined) {
      ...
      if (availability === 'Available') {
        cy.contains(certificateName)
          .get(selectors.certificateName + ' > div > div > div > div')
          .should('have.css', 'background-color', 'rgb(0, 153, 0)')
          .click()
  ...
  certificatesModalEdit(certificateName, certificatePosition, ShareAvailability, sharesAmount, checkORetype, dismissORsaveORclose = '') {
    ...
    switch (checkORetype) {
      case 'type':
        cy.get(selectors.sharesModalInputField)
          .clear()
          .type(sharesAmount, { force: true })
          .then(() => {
