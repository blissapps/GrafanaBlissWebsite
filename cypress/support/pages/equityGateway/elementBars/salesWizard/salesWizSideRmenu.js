...
class salesWizSideRmenu extends BasePage {
  ...
  helpUrlValidation() {
    cy.get('a[data-test-id="sw-right-bar-link-need-help"]').should('have.attr', 'href').and('include', '/help')
    cy.get(selectors.btnHelp).click()
    cy.url().should('contain', '/help')
  }

  securitySummary(elements) {
    cy.get(selectors.securitySummaryStepper).should('be.visible')
  ...
  shareGroupShareGroupSummary(element) {
    cy.get(selectors.shareGroupSummaryStepper).should('be.visible')
  ...
  amount2SellShareGroupSummary(elements) {
    cy.get(selectors.amount2SellSummaryStepper).should('be.visible')
  ...
  oderTypeShareGroupSummary(element) {
    cy.get(selectors.orderTypeSummaryStepper).should('be.visible')
