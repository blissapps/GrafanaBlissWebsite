...
class salesWizOverviewPage extends BasePage {
  ...
  pageSharesValidation(elements, percentAvailable, percentAvailableWithRestrict, percentUnavailable, percentInProgress) {
    ...
    cy.get(selectors.sectionShares2SellGraph + '.eg-graph--green')
      .invoke('attr', 'style')
      .should('contain', `width: ${percentAvailable}%`)
    cy.get(selectors.sectionShares2SellGraph + '.eg-graph--yellow')
      .invoke('attr', 'style')
      .should('contain', `width: ${percentAvailableWithRestrict}%`)
    cy.get(selectors.sectionShares2SellGraph + '.eg-graph--orange')
      .invoke('attr', 'style')
      .should('contain', `width: ${percentUnavailable}%`)
    cy.get(selectors.sectionShares2SellGraph + '.eg-graph--blue')
      .invoke('attr', 'style')
      .should('contain', `width: ${percentInProgress}%`)
  }
